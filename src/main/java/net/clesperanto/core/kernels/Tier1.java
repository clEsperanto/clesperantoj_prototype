...
public class Tier1 {
	 ...
	 * @return the output image. An {@link ArrayJ} object containing the resultant
	 *         image after applying the absolute value
	 *         transformation. The output image will have the same dimensions and
	 *         type as the input image.
	 * @throws NullPointerException if the input image is {@code null}.
	 */
	public static ArrayJ absolute(ArrayJ input, ArrayJ output) {
	 ...
	 *               the input image to be processed
	 * @param output
	 *               the output image where results are written into. Cannot be
	 *               null, the result is written into this object.
	 ...
	 * </pre>
	 *
	 * The function supports various input types including {@link ArrayJ}, ImageJ
	 * ImagePlus, ImagLib2 RandomAccessible,
	 ...
	 * @return the resulting object. It is returned in the same type as the object.
	 *         If the input was an {@link ArrayJ},
	 *         the output will be an {@link ArrayJ}, and so on.
	 * @throws NullPointerException     if input is null.
	 * @throws IllegalArgumentException if the input is an unsupported Object. This
	 *                                  can happen because the Object is
	 *                                  not supported or because the dependencies
	 *                                  needed for the object are missing
	 */
	 ...
	 * any additional dependencies.
	 *
	 * This method always return an {@link ArrayJ} as the output. Useful if we want
	 * to do another operation with
	 * ClesperantoJ as the {@link ArrayJ} stays on the device (GPU) and does not
	 * need to be copied back and forth
	 ...
	 *                   Device to perform the operation on.
	 * @param input
	 *                   the input image to be processed. Can be any of the
	 *                   supported types: {@link ArrayJ}, ImageJ ImagePlus,
	 *                   ImagLib2 RandomAccessible, or Icy Sequence. For types other
	 *                   than {@link ArrayJ}, corresponding
	 *                   dependencies must be included.
	 * @param objectType
	 *                   the {@link ObjectType} in which we want the result to be
	 *                   given. If {@code null}, it defaults to
	 *                   {@link ObjectType#ARRAYJ} producing an {@link ArrayJ}
	 * @return the result of the processing on the input. The object type depends on
	 *         the one selected. Note that
	 *         in order to produce something different to {@link ArrayJ} the
	 *         required dependencies need to be present
	 * @throws NullPointerException     if input is null.
	 * @throws IllegalArgumentException if the input is an unsupported Object. This
	 *                                  can happen because the Object is
	 *                                  not supported or because the dependencies
	...
	public static ArrayJ absolute(DeviceJ device, Object input, ArrayJ output) {
		...
		//// encapsulate into a function and return an ArrayJ
		if (DepsManager.IMGLIB2_AVAILABLE && DepsManager.RAI_CLASS.isInstance(input)) {
			// we convert input into ArrayJ (equivalent to push)
		} else if (DepsManager.IMAGEPLUS_AVAILABLE && DepsManager.IMAGEPLUS_CLASS.isInstance(input)) {
			// we convert input into ArrayJ (equivalent to push)
		} else {
			throw new IllegalArgumentException("Unsupported input object: " + device.getClass().toString());
		...
		if (output == null) {
			return ArrayJ(net.clesperanto._internals.kernelj.Tier1.absolute(input.getDevice().getRaw(),
					((ArrayJ) input).getRaw(), null));
		}
		return ArrayJ(net.clesperanto._internals.kernelj.Tier1.absolute(input.getDevice().getRaw(), input.getRaw(),
				output.getRaw()));
	}

	// last version is the best one, with the follow up modifications:
