...
public class Tier1 {
	...
	/**
	 * Convolve the image with a gaussian derivate.
	 * The filter kernel can have nonisotropic sigma and order.
	 * The implementation is done separable.
	 ...
	 * @throws NullPointerException if any of the device or input parameters are null.
	 */
    public static ArrayJ unpadFunc(DeviceJ device, ArrayJ input, ArrayJ output, int size_x, int size_y, int size_z, boolean center) {
        Objects.requireNonNull(device, "device cannot be null");
		Objects.requireNonNull(input, "input cannot be null");
        return new ArrayJ(net.clesperanto._internals.kernelj.Tier1.unpad_func(device.getRaw(), input.getRaw(), output == null ? null : output.getRaw(), size_x, size_y, size_z, center));
    }
